...
for version in $versions_to_build; do
    ...
    [ -z "${PULL:-}" ] || docker pull "nholuongut/presto-dev:$version"
    docker build -t "nholuongut/presto-dev:$version" --build-arg PRESTO_VERSION="$version" $no_cache .
    [ -n "${NOPUSH:-}" ] || docker push "nholuongut/presto-dev:$version"
    ...
    if [ $count -gt 1 ] && [ -z "${NODELETE:-}" ]; then
        docker rmi "nholuongut/presto-dev:$version"
    fi
    echo
done
